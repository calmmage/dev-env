...
def deep_diff(d1, d2):
    ...
    if isinstance(d1, list) and isinstance(d2, list):
        ...
        return (
            list_diff if list_diff["added"] or list_diff["removed"] or list_diff["changed"] else {}
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compare macOS format between two states")
    parser.add_argument(
        "--before_file",
    ...
    parser.add_argument(
        "-o", "--output", default="output/settings_diff.json", help="Output JSON file for the diff"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Print verbose processing information"
    )
